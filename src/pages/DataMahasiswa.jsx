    ...
    <div className="min-h-screen pt-32 pb-20 bg-slate-50 dark:bg-slate-950">
      <div className="container-custom">
        <div className="max-w-4xl mb-16 text-center mx-auto">
          ...
          >
            {t('mahasiswa.data.title')} <span className="gradient-text">{t('mahasiswa.data.title_accent')}</span>
          </m.h1>
          <p className="text-lg text-slate-600 dark:text-slate-400">
            {t('mahasiswa.data.subtitle')}
          </p>
        </div>
          ...
          >
            <div className="relative z-10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                <div>
                  <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-2">{t('mahasiswa.stats.title')}</h3>
                  <p className="text-sm text-slate-500 font-medium">{t('mahasiswa.stats.subtitle')}</p>
                </div>
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full bg-primary-600" />
                    <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{t('mahasiswa.stats.label')}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-4 justify-center">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-primary-600" />
                  <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{t('mahasiswa.data.ganjil')}</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-indigo-400" />
                  <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{t('mahasiswa.data.genap')}</span>
                </div>
              </div>
            </div>
        ...
        <div className="flex items-center justify-center mb-12">
          <div className="inline-flex p-1.5 bg-slate-100 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
            ...
            >
              <Users size={18} />
              {t('mahasiswa.data.directory_tab')}
            </button>
            <button
            ...
            >
              <BookOpen size={18} />
              {t('mahasiswa.data.publications_tab')}
            </button>
          </div>
        </div>
        ...
        <div className="relative max-w-xl mx-auto mb-16">
          ...
          <input
            type="text"
            placeholder={activeTab === 'directory' ? t('mahasiswa.data.search_placeholder_students') : t('mahasiswa.data.search_placeholder_publications')}
            value={searchTerm}
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 text-slate-400">
            ...
            <p className="font-bold uppercase tracking-widest text-[10px]">{t('mahasiswa.stats.loading')}</p>
          </div>
        ) : (
                  ...
                  >
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">{t('mahasiswa.data.batch_label')} {student.batch}</p>
                      </div>
                    </div>
                    ...
                    {student.achievements && student.achievements.length > 0 && (
                      <div className="space-y-3 mt-auto pt-6 border-t border-slate-50 dark:border-slate-800">
                        <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                          ...
                          {t('mahasiswa.data.top_achievements')}
                        </div>
              ...
              >
                {filteredPublications.length > 0 ? (
                      ...
                      <div className="flex-grow">
                        <div className="flex flex-wrap items-center gap-3 mb-4">
                          <span className="text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 text-slate-500">
                            {pub.type || t('mahasiswa.data.publication_type_default')}
                          </span>
                  ...
                  <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-[3rem] border-2 border-dashed border-slate-100 dark:border-slate-800">
                    ...
                    <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">{t('mahasiswa.data.no_publications')}</p>
                  </div>
                )}
              </m.div>
            )}
