    ...
    <div className="min-h-screen pt-32 pb-20 bg-white dark:bg-slate-950 overflow-hidden relative">
      ...
      <div className="container-custom relative z-10">
        <div className="max-w-4xl mx-auto text-center mb-20">
          ...
          >
            <TrendingUp size={16} />
            <span>{t('alumni.tracer.badge')}</span>
          </m.div>
          <m.h1
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            {t('alumni.tracer.title')} <span className="text-emerald-600">{t('alumni.tracer.title_accent')}</span>
          </m.h1>
          <p className="text-xl text-slate-600 dark:text-slate-400 leading-relaxed max-w-3xl mx-auto">
            {t('alumni.tracer.subtitle')}
          </p>
        </div>
        ...
        >
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-black mb-6 leading-tight">{t('alumni.tracer.hero_title')}</h2>
              <p className="text-slate-400 text-lg mb-10 leading-relaxed">
                {t('alumni.tracer.hero_desc')}
              </p>
              <a 
              ...
              >
                {t('alumni.tracer.hero_cta')} <ExternalLink size={20} className="group-hover:rotate-12 transition-transform" />
              </a>
            </div>
            
            <div className="grid grid-cols-2 gap-4">
              {[
                { label: t('alumni.tracer.feature_1'), icon: BarChart3, color: "bg-blue-600" },
                { label: t('alumni.tracer.feature_2'), icon: Target, color: "bg-indigo-600" },
                { label: t('alumni.tracer.feature_3'), icon: Award, color: "bg-amber-600" },
                { label: t('alumni.tracer.feature_4'), icon: Heart, color: "bg-rose-600" },
              ].map((item, i) => (
        ...
        <div className="max-w-5xl mx-auto">
          <h3 className="text-2xl font-black text-center mb-12 text-slate-900 dark:text-white">{t('alumni.tracer.why_title')}</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="p-8 rounded-3xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
              <h4 className="font-black text-emerald-600 mb-4 flex items-center gap-2">
                <ArrowRight size={20} /> {t('alumni.tracer.reason_1_title')}
              </h4>
              <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
                {t('alumni.tracer.reason_1_desc')}
              </p>
            </div>
            <div className="p-8 rounded-3xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
              <h4 className="font-black text-emerald-600 mb-4 flex items-center gap-2">
                <ArrowRight size={20} /> {t('alumni.tracer.reason_2_title')}
              </h4>
              <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
                {t('alumni.tracer.reason_2_desc')}
              </p>
            </div>
          </div>
        </div>
